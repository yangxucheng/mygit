		...
		<div class="sidenav">
			<div id="mydamai" style="">
				...
				<dl>
					<dt>交易中心</dt>
					<dd>
						<a href="<c:url value='/admin/UpdateTickets.jsp'/>" id="nav_Order"
							class="" title="订单管理" domain=""> <span>票项管理</span>
						</a>
					</dd>
					<dd>
...
<div class="right-main">
	...
	用户名:${userInfo.username}&gt; <span>票项管理</span>
	<a class="right-help" href="#">查看帮助>></a></div>
<div class="tab-mydamai">
	<ul class="title">
					<li><a class="on" href="UpdateTickets.jsp">票项修改</a></li>
	...
	<form action="AdminUpdateTicketById" method="post" id="form1">
		<p><font color='red' size="-1"> ${errors}</font>
		<input type="hidden" name="tid" value="${getTicketById.tid }" /><br />
		<br /> 票名：<input type="text" name="descs"
			value="${getTicketById.descs }" /><br />
		<br /> 总票数：<input type="text" name="amount"
			value="${getTicketById.amount }" /><br />
		<br /> 剩余票数：<input type="text" name="balance"
			value="${getTicketById.balance }" /><br />
		<br /> 单价(元)：<input type="text" name="price"
			value="${getTicketById.price }" /><br />
		<br /> 图标路径：<input type="text" name="images"
			value="${getTicketidById.images }" /><br />
		<br /> <input type="button" value="修改" onclick="register()" /> <input
			type="reset" />
	</form>
</body>
